...
const handleUserLogin = async (request, response) => {
  ...
  await body("login_email").isEmail().withMessage("Invalid email").run(request);
...
const handleUserQuery = async (request, reponse) => {
  try {
    const headerAuthentication = request.headers.authentication;
    const accessToken = headerAuthentication.split(" ")[1];
    console.log(accessToken);

    const JWT_SECRET = "abc123";
    const decodedUserData = jwt.verify(accessToken, JWT_SECRET);
